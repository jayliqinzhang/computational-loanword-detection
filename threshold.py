...
## Import the distance values and the cognate list. 
with open ('data/sca_distance.pkl', 'rb') as f:
    distance=pickle.load(f)
        
with open ('data/cognate_list.pickle','rb') as f:
    cognate_list=pickle.load(f)['cognate_list']
 

### a all_ipa_tur_ira_cross list is needed to mark the 0 and 1. 
...
def get_threshold(tup, plot=False):
    ...
    print('the best threshold is value is %s' %best_threshold)
